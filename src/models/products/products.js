const jewelryProducts = [
  {
    id: 1,
    image: "https://picsum.photos/200",
    name: "Blue Swirl Coffin Earrings",
    description: "Handcrafted resin earrings with embedded gold flakes for a subtle shimmer.",
    tag: "earrings",
    price: 24.99
  },
  {
    id: 2,
    image: "https://picsum.photos/200",
    name: "Circle Framed Flowers Necklace",
    description: "Natural cedar wood pendant with a smooth finish on adjustable black cord.",
    tag: "necklace",
    price: 29.99
  },
  {
    id: 3,
    image: "https://picsum.photos/200",
    name: "Ocean Glass Cuff Bracelet",
    description: "Clear resin cuff bracelet with sea-blue swirls inspired by coastal tides.",
    tag: "bracelet",
    price: 19.99
  },
  {
    id: 4,
    image: "https://picsum.photos/200",
    name: "Walnut Halo Ring",
    description: "Minimalist wooden ring made from reclaimed walnut with a polished seal.",
    tag: "ring",
    price: 14.99
  },
  {
    id: 5,
    image: "https://picsum.photos/200",
    name: "Stargazer Key Charm",
    description: "Small resin keychain charm with embedded stars and silver flecks.",
    tag: "other",
    price: 9.99
  },
  {
    id: 6,
    image: "https://picsum.photos/200",
    name: "Rose Petal Hoops Earrings",
    description: "Clear resin hoops containing real dried rose petals for a romantic look.",
    tag: "earrings",
    price: 22.99
  },
  {
    id: 7,
    image: "https://picsum.photos/200/200",
    name: "Maple Grain Beads Bracelet",
    description: "Stretch bracelet made from smooth maple wood beads with a natural finish.",
    tag: "bracelet",
    price: 17.99
  },
  {
    id: 8,
    image: "https://picsum.photos/200",
    name: "Teardrop Quartz Pendant Necklace",
    description: "Necklace with a teardrop-shaped clear resin pendant containing quartz chips.",
    tag: "necklace",
    price: 27.99
  },
  {
    id: 9,
    image: "https://picsum.photos/200",
    name: "Emerald Forest Band Ring",
    description: "Resin ring with swirling green pigments reminiscent of forest moss.",
    tag: "ring",
    price: 13.99
  },
  {
    id: 10,
    image: "https://picsum.photos/200",
    name: "Galaxy Bookmark",
    description: "Resin bookmark with shimmering purple and blue pigment finished with tassel.",
    tag: "other",
    price: 11.99
  }
];

const getAllProducts = () => {
    return jewelryProducts;
};

const getSortedFilteredProducts = (products, sortBy, tags) => {
    
        if (tags && !Array.isArray(tags)) {
            tags = [tags];
        }
       
        if (tags && tags.length > 0) {
            products = products.filter(product =>
                tags.map(t => t.toLowerCase()).includes(product.tag.toLowerCase())
            );
        }
    
        switch (sortBy) {
            case 'priceAsc':
                products.sort((a, b) => a.price - b.price);
                break;
            case 'priceDesc':
                products.sort((a, b) => b.price - a.price);
                break;
            case 'nameAsc':
                products.sort((a, b) => a.name.localeCompare(b.name));
                break;
            case 'nameDesc':
                products.sort((a, b) => b.name.localeCompare(a.name));
                break;
            default:
                break;
        }
        return products;
    };

export { getSortedFilteredProducts };